...
function ImpactChain({ impactChain, artifacts }) {
  const linkedArtifacts = useMemo(
    () => artifacts.filter((artifact) => artifact.traceability === 'linked'),
    [artifacts],
  )
  const [selectedArtifactId, setSelectedArtifactId] = useState(linkedArtifacts[0]?.artifactId || '')
  const activeArtifactId = linkedArtifacts.some(
    (artifact) => artifact.artifactId === selectedArtifactId,
  )
    ? selectedArtifactId
    : linkedArtifacts[0]?.artifactId || ''
  const selectedArtifact = linkedArtifacts.find((artifact) => artifact.artifactId === activeArtifactId)
        ...
        {displayedPath.map((artifact, index) => (
          <div key={artifact.id} className="flex shrink-0 items-center gap-2">
            <article className={`glass-surface w-44 p-4 ${
              artifact.id === activeArtifactId ? 'ring-2 ring-accent/60' : ''
            }`}>
